...
export function ContentCard({ item }: { item: any }) {
  return (
    <Link href={`/content/${item.id}`}>
      <div className="group relative aspect-[2/3] cursor-pointer overflow-hidden rounded-xl bg-secondary ring-1 ring-white/5 transition-all duration-300 hover:-translate-y-1 hover:ring-2 hover:ring-primary/60 hover:shadow-2xl hover:shadow-primary/25">
        {item.posterUrl ? (
          <img
            src={item.posterUrl}
            alt={item.title}
